...
const aircraftOptions = [
  { category: 'Light Jets', suitability: 'Possible', desc: 'Light jets can manage the Amsterdam to Bodrum route but may operate near their range limits depending on the aircraft model.' },
  { category: 'Midsize Jets', suitability: 'Ideal', desc: 'Midsize jets offer the perfect combination of range, cabin comfort, and efficiency for this popular Northern European route.' },
  { category: 'Super-Midsize Jets', suitability: 'Maximum Comfort', desc: 'Super-midsize jets provide stand-up cabins, enhanced amenities, and faster cruising speeds for a premium experience.' },
];

export default function AmsterdamToBodrum() {
  useEffect(() => {
...
window.scrollTo(0, 0);
  ...
  return (
    <main>
      ...
      <section className="py-20 md:py-28 bg-white"><div className="max-w-6xl mx-auto px-5"><div className="text-center mb-14"><span className="text-gold text-sm font-semibold uppercase tracking-[0.25em]">Amsterdam Departures</span><h2 className="font-serif text-3xl md:text-4xl lg:text-5xl text-navy mt-3 mb-4">Departure Airports</h2><div className="w-20 h-px bg-gold mx-auto" /></div><div className="grid md:grid-cols-2 gap-8 max-w-3xl mx-auto">{[{ name: 'Amsterdam Schiphol (AMS)', desc: 'One of Europe\'s busiest airports with excellent private jet facilities, dedicated FBOs, and premium handling services.' }, { name: 'Rotterdam The Hague (RTM)', desc: 'A convenient alternative with efficient private aviation handling and less congestion than Schiphol.' }].map((a, i) => (<div key={i} className="bg-sand p-8 text-center"><span className="text-gold"><svg className="w-10 h-10 mx-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}><path strokeLinecap="round" strokeLinejoin="round" d="M6 12L3.269 3.126A59.768 59.768 0 0121.485 12 59.77 59.77 0 013.27 20.876L5.999 12zm0 0h7.5" /></svg></span><h3 className="font-serif text-lg text-navy font-semibold mt-4 mb-2">{a.name}</h3><p className="text-navy/70 text-sm leading-relaxed">{a.desc}</p></div>))}</div></div></section>

      <section className="py-20 md:py-28 bg-sand"><div className="max-w-6xl mx-auto px-5"><div className="text-center mb-14"><span className="text-gold text-sm font-semibold uppercase tracking-[0.25em]">Bodrum Arrival</span><h2 className="font-serif text-3xl md:text-4xl lg:text-5xl text-navy mt-3 mb-4">Arrival Airport</h2><div className="w-20 h-px bg-gold mx-auto" /></div><div className="max-w-3xl mx-auto"><div className="bg-white p-10 shadow-sm border-t-4 border-gold text-center"><h3 className="font-serif text-2xl md:text-3xl text-navy font-semibold mb-2">Milas-Bodrum Airport (BJV)</h3><p className="text-navy/70 text-lg leading-relaxed mt-4">All private jet arrivals from Amsterdam land at Milas-Bodrum Airport. Private Jet Bodrum coordinates GAT access, apron-side transfers, and direct onward transportation.</p></div></div></div></section>

      <section className="py-20 md:py-28 bg-navy"><div className="max-w-4xl mx-auto px-5 text-center"><div className="w-16 h-px bg-gold mx-auto mb-8" /><h2 className="font-serif text-3xl md:text-4xl lg:text-5xl text-white mb-6">Fly from Amsterdam to <span className="text-gold">Bodrum</span></h2><p className="text-lg md:text-xl text-white/70 max-w-2xl mx-auto leading-relaxed mb-10">Share your travel dates and passenger count. Our concierge team will prepare tailored aircraft options for your Amsterdam to Bodrum journey.</p><a href="/#contact" aria-label="Request Your Amsterdam to Bodrum Quote" className="inline-block bg-gold hover:bg-gold-dark text-navy font-semibold px-10 py-4 text-sm md:text-base uppercase tracking-widest transition-all duration-300 hover:shadow-lg hover:shadow-gold/20">Request Your Amsterdam → Bodrum Quote</a><div className="w-16 h-px bg-gold/40 mx-auto mt-12" /></div></section>
    </main>
  );
}
